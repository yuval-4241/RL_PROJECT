...
\author{
  Daniel Shavit \\
  \texttt{Department of Industrial} \\
  \texttt{Engineering and Management,} \\
  \texttt{Ben Gurion University} \\
  \texttt{shavda@bgu.ac.il} \\
  \And
  Yuval Zohar \\
  \texttt{Department of Industrial} \\
  \texttt{Engineering and Management,} \\
  \texttt{Ben Gurion University} \\
  \texttt{yuvalzoh@post.bgu.ac.il} \\
}
...
extended training causes the proxy reward to be corrupted: the model learns to
...
($\alpha{=}0.3$, $\alpha{=}0.5$), and a ground-truth ceiling condition -- each
trained for 900 steps with periodic held-out evaluation. On the final,
low-noise 100-question evaluation, plain SRT finished training with a
clearly positive agreement-accuracy gap ($+0.043$), the reward-hacking
signature the original paper describes; a moderate entropy bonus
($\alpha{=}0.3$) achieved the best result of the three trained conditions
(highest accuracy, smallest gap), while a stronger bonus ($\alpha{=}0.5$)
underperformed both alternatives -- a non-monotonic effect of $\alpha$.
Surprisingly, the ground-truth
condition did not behave like a ceiling at this training budget: its final
accuracy ($0.25$) was on par with or below all three label-free conditions,
suggesting that reward \emph{density} (how often a rollout receives useful
signal) may matter as much as reward \emph{correctness} within a fixed,
modest training budget. A preliminary frozen-model pilot across three off-the-shelf models
confirms that the agreement-accuracy gap SRT relies on is a real, measurable
phenomenon prior to any training, motivating the intervention studied here.
\end{abstract}
...
needed because the model is, in effect, both ``the student'' and ``the teacher''.
...
instance of reward hacking: a proxy stops being a good measure once it is
directly optimized, and the original paper leaves \emph{fixing} it as an
...
training loop, add a ground-truth condition as an upper-bound
reference the original paper does not report, and run a controlled
comparison across four reward conditions at 900 training steps each --
chosen because reward hacking in the original paper's own experiments only
becomes visible after substantial training.
...
Section~\ref{sec:results} reports results across all four 900-step
conditions. Section~\ref{sec:discussion} discusses limitations and future
work.
...
\textbf{Why sample several rollouts instead of one.} With $k{=}1$, RLOO has
no other rollouts to compare against and collapses to a single raw reward
with no baseline: every sequence is reinforced or discouraged purely on the
sign of its own reward, a high-variance signal that says nothing about
whether the model could have done better on that same prompt. Sampling
$k>1$ rollouts turns each one into a \emph{relative} comparison against its
peers, which sharpens the gradient (a rollout is only reinforced if it beats
the average of the others, not merely if it is ``good'' in isolation) and
removes the need for a learned critic. Multiple rollouts per prompt are also
a hard requirement for SRT specifically, independent of RLOO: the
majority-vote reward is only definable when there is a crowd to agree or
disagree with, so with $k{=}1$ there is no self-consistency signal to train
on at all.
...
For a training question, the policy generates $k$ rollouts with final
answers $a_1,\dots,a_k$. Not every rollout necessarily yields an answer that
can be parsed: if a rollout's output does not contain a valid
\verb|\boxed{...}| expression (e.g.\ it was truncated, or the model never
committed to a final answer), that rollout's answer is recorded as
\texttt{None} and excluded from scoring. Let $p(a)$ be the empirical
fraction \emph{among the parseable rollouts only} (i.e.\ excluding any
\texttt{None}s) that produced answer $a$. We define the per-rollout reward
\begin{equation}
...
\subsection{The $\alpha$ hyperparameter: a derivable bound, not a guess}
\label{sec:alpha-bound}

The strength of the bonus, $\alpha$, controls how much the model is allowed
to value \emph{being different} over \emph{being in the majority}, and this
turns out to have a sharp, derivable threshold rather than only being a
tuning knob to sweep blindly. Among $k$ rollouts, the rarest a parseable
answer can be is $p(a) = 1/k$ (a single rollout out of $k$ produced it), so
the largest surprisal bonus any rollout can ever earn is
$-\log(1/k) = \log k$. Because matching the majority is worth exactly $1.0$
(Eq.~\ref{eq:reward}), a maximally rare, disagreeing rollout begins to
out-earn a majority-matching rollout once $\alpha \log k > 1$, i.e.\ once
\[
\alpha \;>\; \alpha^{*} \;=\; \frac{1}{\log k}.
\]
For our $k{=}8$ rollout budget, $\log 8 \approx 2.079$, giving
$\alpha^{*} \approx 0.481$. Below this threshold, agreeing with the majority
is \emph{always} the higher-reward choice, even in the worst case where the
disagreeing rollout is maximally rare; above it, a sufficiently rare
disagreement can outscore agreement outright. Our two tested nonzero values
sit on opposite sides of this boundary: $\alpha{=}0.3 < \alpha^{*}$, so the
bonus can only ever partially offset -- never overturn -- the incentive to
agree with the crowd, meaning it protects rare-but-plausible answers from
being fully starved of reward without threatening the majority-vote signal
itself. $\alpha{=}0.5 > \alpha^{*}$, so a lone, maximally rare rollout can
occasionally out-earn the majority, meaning the bonus is strong enough to
actively compete with -- not just cushion -- the self-consistency signal SRT
is built on. This gives a mechanistic, before-the-fact explanation for what
the surprisal bonus changes about what the model learns: below $\alpha^{*}$
it mainly changes \emph{how harshly} disagreement is punished; above it, it
can change \emph{which answer is worth producing at all}, since the
optimal policy under the reward is no longer guaranteed to be the majority
answer. Section~\ref{sec:results} reports the resulting non-monotonic
pattern in final accuracy and gap, which this bound anticipates. We note
this is a worst-case, single-rollout bound rather than an empirical estimate
from our actual answer-frequency distributions -- most rollouts do not sit
at the extreme $p(a){=}1/k$ tail, so the bound is a conservative one, derived
to explain the observed pattern rather than predicted in advance of running
the experiment.

\subsection{Ground-truth ceiling}

We additionally implement a \textbf{ground-truth} condition,
...
\begin{itemize}
  \item \textbf{$H_0$ (collapse under budget):} given enough training steps,
  ...
  model scale.
  \item \textbf{$H_1$ (bonus delays collapse):} $\alpha > 0$ conditions will
...
\end{itemize}
Both hypotheses are evaluated in Section~\ref{sec:results} against the
completed 900-step runs.
...
no real answer is available to the training loop except in the ground-truth
...
exposed to reward computation in the other conditions. Evaluation uses a disjoint,
...
entropy-bonus / ground-truth rewards) in a single, dependency-minimal script rather
...
and score answers; compute rewards per Eq.~\ref{eq:reward} (or the ground-truth
...
proposed configuration), and the ground-truth condition. Held-out evaluation
...
900 steps was chosen as the largest training budget practical within the
project's compute allocation, guided by the original paper's own
observation that reward-hacking collapse is a phenomenon of
\emph{extended} training rather than early training -- we wanted a horizon
long enough to give that failure mode a realistic chance to appear, if it
occurs at this much smaller model scale.
...
All four conditions -- $\alpha{=}0.0$, $\alpha{=}0.3$, $\alpha{=}0.5$, and
the ground-truth condition -- have completed their full 900-step runs, each
with a final evaluation over all 100 held-out test questions.

\begin{table}[h]
  \caption{Final (100-question) evaluation after 900 training steps, by
  condition. Agreement is derived as gap $+$ accuracy.}
  ...
  \begin{tabular}{lccc}
    ...
    $\alpha = 0.3$                  & \textbf{0.30} & \textbf{0.32} & \textbf{$+0.016$} \\
...
\begin{figure}[h]
  \centering
  \includegraphics[width=0.98\linewidth]{results_curves.png}
  \caption{Cumulative (running) average of test accuracy (left) and
  agreement-accuracy gap (right) vs.\ training step, for all four
  conditions, including the ground-truth condition. Each bold line is the running mean of
  that condition's 20-question in-training checkpoints (every 15 steps,
  fixed seed) from step 0 up to the current step; faint dots are the raw,
  unsmoothed checkpoints, shown to make the per-checkpoint sampling noise
  visible rather than hiding it. Gap is agreement minus accuracy
  (Section~\ref{sec:background}): positive means the model agrees with
  itself more than it is actually correct, the reward-hacking signature.
  The low-noise, 100-question final evaluation for each condition is
  reported separately in Table~\ref{tab:main-results} rather than
  overlaid here.}
  \label{fig:results-curves}
\end{figure}

Table~\ref{tab:main-results} and Figure~\ref{fig:results-curves} show two
results worth highlighting.

\textbf{First, plain SRT ends training with a clearly positive gap, though
the in-training trend alone would not have predicted it.} On the
low-noise, 100-question final evaluation, $\alpha{=}0.0$ reached $0.26$
accuracy with a $+0.043$ gap -- the second-largest (least healthy) gap of
the four conditions (Table~\ref{tab:main-results}). A positive gap of this
size is broadly consistent with \textbf{$H_0$}: it is the signature the
original paper associates with reward-hacking collapse. However, this is
essentially invisible in the in-training signal itself: the blue
running-average curve in Figure~\ref{fig:results-curves} (right panel)
stays close to $-0.03$ for the entire 900 steps and never approaches the
$+0.043$ figure ultimately reported in Table~\ref{tab:main-results}.
Individual raw 20-question checkpoints (faint dots) do occasionally spike
as high as the final value or higher, but they spike below it just as
often, so their running average never resolves a clear direction; the
noise floor of a 20-question estimate (roughly $\pm0.05$--$0.10$) is simply
too large relative to the effect size to detect it from the training-time
signal alone. Because we only have one large-sample, low-noise measurement
per condition -- taken once, at the very end of training -- we cannot tell
from this run alone whether the gap grew gradually and the small-sample
trend simply lacked the resolution to see it, or whether the final number
is closer to a step-900-specific reading than a stable trend. We treat this
as a genuine open question rather than settled evidence of collapse; a
methodological point future work in this space should account for when
sizing evaluation sets (Section~\ref{sec:discussion}).

\textbf{Second, the entropy bonus's effect on the gap was non-monotonic in
$\alpha$, only partially supporting $H_1$.} $\alpha{=}0.3$ was the
best-performing trained condition on every axis measured: highest final
accuracy ($0.30$, versus $0.26$ for plain SRT and $0.24$ for $\alpha{=}0.5$)
and the smallest gap ($+0.016$, versus $+0.043$ and $+0.051$ respectively).
This is the pattern $H_1$ predicts: a moderate entropy bonus reduced the
reward-hacking signature relative to plain SRT. $\alpha{=}0.5$ did not
continue this trend, however -- it was the \emph{worst} of the three
conditions on both accuracy and gap. A bonus that is too strong appears to
hurt rather than help: at $\alpha{=}0.5$ the surprisal term may compete
enough with the
majority-vote term to weaken the self-consistency signal SRT depends on,
rather than merely preventing its collapse. Taken together, the three
conditions suggest an inverted-U relationship between $\alpha$ and final
performance rather than a monotonically increasing benefit of larger
bonuses. This lines up with the $\alpha^{*} \approx 0.481$ bound derived in
Section~\ref{sec:alpha-bound}: $\alpha{=}0.3$ sits below it (the bonus can
only ever cushion the majority-vote signal), while $\alpha{=}0.5$ sits above
it (the bonus can occasionally overturn it) -- a mechanistic account of why
more bonus was not simply better.

\textbf{Third, the ground-truth condition did not behave like a ceiling at this
training budget -- it was, if anything, one of the weaker conditions.}
Trained directly on ground-truth reward ($R_i = \mathbf{1}[a_i =
\text{ground truth}]$, no majority vote involved at all), this condition
reached only $0.25$ final accuracy: below $\alpha{=}0.3$, and statistically
indistinguishable from plain SRT and $\alpha{=}0.5$ given the
$\pm0.04$--$0.045$ noise floor discussed in Section~\ref{sec:discussion}.
This is not what Section~\ref{sec:contribution} predicted the ground-truth
condition would show. A plausible explanation is reward \emph{sparsity}: early in training,
the base model is rarely exactly correct, so this condition's binary
$0$/$1$ reward is $0$ for nearly every rollout, giving RLOO very little
useful gradient signal per step. The majority-vote reward, by contrast, is
$1$ for roughly half of all $8$ rollouts \emph{by construction} (whichever
answer happens to be most common), regardless of whether that answer is
correct -- a denser, if noisier, training signal. Within a fixed, modest
900-step budget, that density advantage may matter more than the majority
vote's lack of correctness guarantee. A second, equally important
observation: the ground-truth condition's own gap ($+0.048$) is not near zero, even though
its reward never references the majority vote at all. Since \emph{gap} was
introduced (Section~\ref{sec:background}) specifically as a majority-vote
reward-hacking signature, a positive gap under ground-truth training
suggests the metric also reflects something more generic about this model
and training scale -- general RL noise or calibration drift, for instance
-- and should not be read as proof of majority-vote-specific reward hacking
on its own.

\subsection{Comparison against a training-free baseline}

Section~\ref{sec:alpha-bound} explains \emph{why} $\alpha{=}0.3$ and
$\alpha{=}0.5$ diverge; Figure~\ref{fig:alpha-baseline} asks a different
question -- how much either trained condition actually bought over doing no
training at all. We compare the two trained conditions against two
reference points measured on the untrained base model before any gradient
update: its zero-shot (single-sample) accuracy, and its 8-rollout
majority-vote accuracy, both from the same fixed evaluation subset used for
in-training checkpoints (Section~\ref{sec:methodology}).

\begin{figure}[h]
  \centering
  \includegraphics[width=0.55\linewidth]{alpha_baseline.png}
  \caption{Untrained zero-shot and untrained majority-vote accuracy (gray /
  light blue) versus final accuracy after 900 steps of training at
  $\alpha{=}0.3$ and $\alpha{=}0.5$ (green / red). The untrained bars use the
  fixed 20-question in-training subset; the trained bars use the full
  100-question held-out set (Section~\ref{sec:methodology}), so the
  comparison is directional rather than exact.}
  \label{fig:alpha-baseline}
\end{figure}

Simply taking a majority vote over 8 untrained rollouts raises accuracy from
$0.05$ (zero-shot) to $0.30$ -- consistent with
\citet{wang2022selfconsistency}'s original finding that self-consistency
voting is a strong accuracy booster on its own, with no training involved.
After 900 steps of RL training, $\alpha{=}0.3$ reaches $0.30$ final accuracy
-- matching, rather than clearly exceeding, what untrained majority voting
already provided for free, while $\alpha{=}0.5$ falls below it at $0.24$.
Because the untrained baseline is measured on a smaller question subset than
the trained conditions (Section~\ref{sec:methodology}), this is a rough
comparison, not a controlled one; still, it is a useful sanity check that at
this model scale and training budget, neither trained condition clearly
outperformed simply asking the untrained model to vote among its own
rollouts -- a different angle on the same reward-density story as the
ground-truth result above.
...
The pattern in Section~\ref{sec:results} -- plain SRT degrading with
extended training while a moderate entropy bonus improves and a strong
bonus overshoots -- is consistent with the mechanism proposed in
Section~\ref{sec:contribution}: a bonus large enough to keep some reward on
rare answers helps resist collapse, but a bonus large enough to materially
compete with the majority-vote term for reward budget can itself destabilize
the self-consistency signal SRT depends on, rather than simply protecting
it. We treat this as a tentative rather than conclusive reading. Each of the
three final accuracy numbers ($0.24$, $0.26$, $0.30$) comes from a single
900-step run with no repeated seeds, and the standard error of a
100-question accuracy estimate at this base rate is on the order of
$\pm0.04$--$0.045$ -- comparable in size to the differences between
conditions. The pattern is directionally consistent with $H_0$ and
(partially) $H_1$, but we cannot rule out that a repeated run with a
different seed would narrow or reverse the ordering between $\alpha{=}0.3$
and $\alpha{=}0.0$/$0.5$. We had hoped the ground-truth condition would help
settle this by providing a clean upper bound to calibrate against; instead
it complicated the picture, landing among the weaker conditions rather than
above all of them (Section~\ref{sec:results}). That result is useful in its
own right -- it argues against reading \emph{any} single 900-step,
single-seed run (ground truth included) as a precise, trustworthy point estimate
at this model scale -- but it means multi-seed replication (see Limitations
below), not the ground-truth condition alone, is what would actually be needed to settle
the ordering with confidence.
...
Sheikh Shafayat, Fahim Tajwar, Ruslan Salakhutdinov, Jeff Schneider, and
Andrea Zanette.
...
\item \textbf{Claims}\\
    ...
    Answer: \answerYes{}\\
    Justification: The abstract and introduction state the observed pattern
    (plain SRT ending training with a clearly positive gap; a moderate
    entropy bonus performing best of the four conditions; a stronger bonus
    underperforming; the ground-truth condition unexpectedly failing to behave as a ceiling)
    and match Section~\ref{sec:results}. All
    four conditions have completed 900-step training as of this draft; the
    single-seed caveat on how confidently these claims generalize is stated
    in Section~\ref{sec:discussion}, not glossed over.
...
\item \textbf{Experiment statistical significance}\\
    ...
    Answer: \answerNo{}\\
    Justification: Each condition was run with a single seed due to
    compute/time constraints; no repeated-seed error bars are reported.
    Section~\ref{sec:discussion} estimates the standard error of the
    100-question accuracy figures ($\approx\pm0.04$) is comparable in size
    to the differences observed between conditions, so the ordering
    between $\alpha$ values should be read as suggestive rather than
...
\item \textbf{Experiments compute resources}\\
    ...
    Answer: \answerYes{}\\
    Justification: Training used two compute environments: a shared lab
    server (NVIDIA RTX 5090) and rented cloud GPUs via RunPod, with the
    specific instance varying by availability across an RTX 4090, an RTX
    PRO 4500, and an RTX 5090 (Section~\ref{sec:methodology}).
    \pending{total wall-clock / GPU-hours across all four 900-step runs not
    yet tallied}.
...
\item \textbf{Declaration of LLM usage}\\
    ...
    Answer: \answerYes{}\\
    Justification: Qwen2.5-Math-1.5B (and, separately, Qwen2.5-32B /
    Llama-4-Scout / o4-mini in the frozen-model pilot) are the
    \emph{subject} of the research, not a tool used to produce it. Separately,
    an LLM coding assistant was used to help write portions of the training
    / analysis code and this report; all LLM-assisted code and text were
    reviewed, tested, and edited by the authors, who take full
    responsibility for correctness of the final method, results, and
    claims. The LLM was not used to generate or alter any experimental
result.
