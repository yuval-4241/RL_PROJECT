...
low-noise 100-question evaluation, plain SRT's accuracy fell and its
agreement-accuracy gap grew substantially relative to an earlier, shorter
feasibility run, consistent with the reward-hacking collapse the original
paper describes; a moderate entropy bonus ($\alpha{=}0.3$) achieved the best
result of the three trained conditions (highest accuracy, smallest gap),
while a stronger bonus ($\alpha{=}0.5$) underperformed both alternatives --
a non-monotonic effect of $\alpha$. Surprisingly, the ground-truth oracle
...
the ground-truth oracle -- have completed their full 900-step runs, each
...
\begin{table}[h]
  ...
  \begin{tabular}{lccc}
    ...
    Oracle (ground-truth reward)    & 0.25          & 0.30          & $+0.048$ \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{figure}[h]
  ...
  conditions, including the oracle. Each bold line is the running mean of
...
\textbf{Third, the oracle condition did not behave like a ceiling at this
training budget -- it was, if anything, one of the weaker conditions.}
Trained directly on ground-truth reward ($R_i = \mathbf{1}[a_i =
\text{ground truth}]$, no majority vote involved at all), the oracle
...
This is not what Section~\ref{sec:contribution} predicted the oracle would
show. A plausible explanation is reward \emph{sparsity}: early in training,
the base model is rarely exactly correct, so the oracle's binary
...
observation: the oracle's own gap ($+0.048$) is not near zero, even though
...
and $\alpha{=}0.0$/$0.5$. We had hoped the oracle condition would help
...
single-seed run (oracle included) as a precise, trustworthy point estimate
at this model scale -- but it means multi-seed replication (see Limitations
below), not the oracle alone, is what would actually be needed to settle
...
\item \textbf{Claims}\\
    ...
    underperforming; the oracle unexpectedly failing to behave as a ceiling)
