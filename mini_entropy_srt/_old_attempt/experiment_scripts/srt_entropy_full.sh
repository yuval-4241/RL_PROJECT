#!/usr/bin/env bash
# Step 5: Scale up — full DAPO dataset with tuned alpha.
# Run after the 100-sample alpha sweep proves which alpha works best.
#
# Usage:
#   bash experiment_scripts/srt_entropy_full.sh 2>&1 | tee entropy_full.txt
#
# To override alpha from command line:
#   bash experiment_scripts/srt_entropy_full.sh reward_model.entropy_alpha=0.2
...
# Entropy-augmented reward manager — set alpha to best value from Step 3
export REWARD_MANAGER='entropy_augmented'
export ENTROPY_ALPHA=0.1         # <-- change to winner from 100-sample sweep
...
# Full DAPO easy-one-third dataset (Step 5 target)
...
EXPERIMENT_NAME=SRT_entropy_alpha${ENTROPY_ALPHA}_full
...
python3 -m verl.trainer.main_ppo \
    ...
    reward_model.entropy_alpha=$ENTROPY_ALPHA \
