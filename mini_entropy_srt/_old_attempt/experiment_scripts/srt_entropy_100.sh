#!/usr/bin/env bash
# Step 3: Entropy-augmented SRT on the 100-sample DAPO test set.
# Runs alpha sweep: A=0.01, B=0.1, C=0.5
#
# Usage:
#   bash experiment_scripts/srt_entropy_100.sh          # runs all three alphas
#   bash experiment_scripts/srt_entropy_100.sh --alpha 0.1   # run one specific alpha
...
# --- Entropy reward manager ---
export REWARD_MANAGER='entropy_augmented'
...
run_alpha() {
    local alpha=$1
    local EXPERIMENT_NAME="SRT_entropy_alpha${alpha}_100"
    local CHECKPOINT_SAVE_PATH=$HOME/self_labeling_checkpoints/${PROJECT_NAME}/${EXPERIMENT_NAME}
    rm -rf $CHECKPOINT_SAVE_PATH

    echo "========================================"
    echo "Run ${EXPERIMENT_NAME}  (alpha=${alpha})"
    echo "========================================"
    ...
    python3 -m verl.trainer.main_ppo \
        ...
        reward_model.entropy_alpha=$alpha \
    ...
    echo "Finished alpha=${alpha} → log in ${EXPERIMENT_NAME}.txt"
}

if [[ -n "$ALPHA_OVERRIDE" ]]; then
    run_alpha $ALPHA_OVERRIDE
else
    # Run A, B, C from Step 3 of the plan
    run_alpha 0.01   # Run A: very weak entropy
    run_alpha 0.1    # Run B: moderate entropy (recommended)
    run_alpha 0.5    # Run C: strict entropy
fi
